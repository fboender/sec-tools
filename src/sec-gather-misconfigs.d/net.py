...
def tcp_sycookies_disabled():
    result = Result(
        ...
        explanation="TCP SYN cookie is a technique used to resist SYN flood attacks",
        severity=2,
        passed=False
    )
...
def icmp_ignore_broadcast():
    result = Result(
        ...
        explanation="Machines should not reply to ICMP ping broadcast requests.",
        severity=1,
        passed=False
    )
...
def icmp_redirects():
    result = Result(
        ...
        explanation="ICMP redirects are used by routers to tell the server that there is a better path to other networks than the one chosen by the server. However, an intruder could potentially use ICMP redirect packets to alter the hosts's routing table by causing traffic to use a path you didn't intend.",
        severity=3,
        passed=False
    )
